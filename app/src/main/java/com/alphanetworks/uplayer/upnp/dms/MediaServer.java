...
public class MediaServer {
	...
	public MediaServer(Context context, InetAddress address) {
		DeviceType type = new UDADeviceType(UpnpUtils.DMS, UpnpUtils.VERSION);
		DeviceDetails details = new DeviceDetails(
				UpnpUtils.DMS_NAME + "(" + android.os.Build.MODEL + ")",
				new ManufacturerDetails(UpnpUtils.MANUFACTURER,
						UpnpUtils.MANUFACTURER_URL),
				new ModelDetails(UpnpUtils.DMS_MODEL_NAME,
						UpnpUtils.DMS_MODEL_DESCRIPTION, UpnpUtils.MODEL_NUMBER));
		...
		try {
			...
			mLocalDevice = new LocalDevice(new DeviceIdentity(UpnpUtils.S_UDN),
					type, details,
			...
			Thread thread = new Thread(new Runnable() {
				...
				public void run() {
					...
					try {
						...
						mHttpServer = new HttpServer(UpnpUtils.PORT);
						if (mHttpServer != null)
							mHttpServer.start();
					} catch (IOException e) {
	...
	public String getAddress() {
		...
		if (mLocalAddress != null)
			return mLocalAddress.getHostAddress() + ":" + UpnpUtils.PORT;
		else
			return null;
	}
