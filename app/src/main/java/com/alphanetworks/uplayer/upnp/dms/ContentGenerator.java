...
/**
 * 
 * 通过扫描MediaStore，得到媒体内容(目前只针对music)的详细信息，并将这些媒体内容 按照一定的逻辑结构添加到本地dms上。 逻辑结构如下：
 * 
 * Album 按照专辑名称分类 / music -- Artist 按照歌手名称分类 \ Track 将所有music放在该目录下
 * 
 * @author 20001962
 *
 */
	...
	public static void prepareAudio(final Context context,
		...
		Thread thread = new Thread(new Runnable() {
			...
			public void run() {
				...
				if (mIsprepared)
					return;
				UpnpUtils.generateContainer(UpnpUtils.CONTENT_AUDIO_ID,
						"Music", UpnpUtils.CONTENT_ROOT_ID, "object.container",
						true);
				UpnpUtils.generateContainer(UpnpUtils.CONTENT_VIDEO_ID,
						"Video", UpnpUtils.CONTENT_ROOT_ID, "object.container",
						true);
				UpnpUtils.generateContainer(UpnpUtils.CONTENT_IMAGE_ID,
						"Image", UpnpUtils.CONTENT_ROOT_ID, "object.container",
						true);

				UpnpUtils.generateContainer(UpnpUtils.AUDIO_Album_ID, "Album",
						UpnpUtils.CONTENT_AUDIO_ID, "object.container", true);

				UpnpUtils.generateContainer(UpnpUtils.AUDIO_Artist_ID,
						"Artist", UpnpUtils.CONTENT_AUDIO_ID,
						"object.container", true);

				Container allTrackContainer = UpnpUtils.generateContainer(
						UpnpUtils.AUDIO_TRACK_ID, "Track",
						UpnpUtils.CONTENT_AUDIO_ID, "object.container", true);
					...
					if (cursor.moveToFirst()) {
						do {
							...
							MusicTrack musicTrack = new MusicTrack(id,
									UpnpUtils.AUDIO_Album_ID, title, artist,
							...
							if (!ContentTree.hasContentNode(album, true)) {
								Container container = UpnpUtils
										.generateContainer(
												album,
												album,
												UpnpUtils.AUDIO_Album_ID,
